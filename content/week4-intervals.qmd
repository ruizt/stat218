...
abline(v = mle[1]/mle[2], col = 'red')
text(x = mle[1]/mle[2] + 0.75, y = 0.425, expr(mu), col = 'red')
...
title(xlab = expression(frac(bar(x) - mu, s[x]/sqrt(n))), line = 4)
...
- `r 100*round(pt(1, 19), 3)`% of samples give values of $T$ under 1

```{r, echo = T}
pt(1, df = 20 - 1) # area less than 1
```

- `r 100*round(pt(1, 19), 3) - 50`% of samples give values of $T$ between 0 and 1
...
*With 95% confidence, the mean total HDL cholesterol is estimated to be between `r round(mean(cholesterol) - 2*sd(cholesterol)/sqrt(50), 3)` and `r round(mean(cholesterol) + 2*sd(cholesterol)/sqrt(50), 3)` mmol/L.*
