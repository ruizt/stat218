...
::: {.column}
The pairwise significance tests pertain to the hypotheses:

$$\begin{cases} H_0: \mu_i - \mu_j = 0 \\ H_A: \mu_i - \mu_j \neq 0 \end{cases}$$
Test statistic:

$$T_{ij} = \frac{\bar{x}_i - \bar{x}_j}{SE_{ij}}
\quad\text{on}\quad (n - k) \text{ df}$$

Directional hypotheses or nonzero null differences can also be tested, but are less common (see lab for implementation).
...
Critical value $c$ chosen according to correction method.

One-sided intervals are possible but not common (see lab for implementation).
